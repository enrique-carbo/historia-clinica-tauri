...
use crate::crypto;
use crate::lib_types::{CryptoState, DbState, SigningState};
use rusqlite::params;
use serde::Serialize;
...
pub fn create_note(
    ...
    // Validación dinámica contra el template
    ...
    // FIRMA DIGITAL con payload dinámico según template
...
pub fn get_note(
    ...
    let mut fields: NoteFields = HashMap::new();
    ...
    let is_verified = match signature_hex {
        Some(sig) => {
            ...
            let pub_key_result: Result<String, _> = super::with_conn(&db_state, |conn| {
                conn.query_row(
