      ...
      <div className="col-span-8 flex flex-col gap-4 overflow-hidden">
        {patient ? (
          <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      {/* ALERTA DE ALERGIA — lee de clinical separado */}
              ...
              {isPatientDetailsOpen && (
                <div className="p-5 pt-4 border-t border-zinc-800 bg-zinc-950/30 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3 pl-1">
                      Antecedentes Clínicos
                    </h4>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { label: "Antecedentes Personales", value: clinical?.antecedentes_personales, color: "text-blue-500" },
                        { label: "Antecedentes Familiares", value: clinical?.antecedentes_familiares, color: "text-blue-500" },
                        { label: "Medicación habitual", value: clinical?.medicacion, color: "text-orange-500" },
                        { label: "Alergias", value: clinical?.alergias, color: "text-red-500", highlight: true },
                      ].map((item, idx) => (
                        <div
                          key={idx}
                          className={`bg-zinc-900/50 rounded-lg p-4 border border-zinc-800/50 min-h-25 ${
                            item.highlight && item.value ? "border-red-900/50 bg-red-900/5" : ""
                          }`}
                        >
                          <div className={`text-[10px] uppercase font-bold tracking-wider mb-2 flex items-center gap-2 ${item.color}`}>
                            {item.highlight && item.value && <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>}
                            {item.label}
                          </div>
                          <p className="text-sm text-zinc-300 whitespace-pre-wrap leading-relaxed">
                            {item.value || <span className="text-zinc-600 italic">Sin registros</span>}
                          </p>
                        </div>
                      ))}
                      <div className="col-span-2 sm:col-span-1 bg-zinc-900/50 rounded-lg p-4 border border-zinc-800/50 flex flex-col justify-center">
                        <div className="text-[10px] uppercase font-bold tracking-wider mb-1 text-emerald-500">Grupo Sanguíneo</div>
                        <div className="text-2xl font-bold text-zinc-200">{clinical?.grupo_sanguineo || "Desconocido"}</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {/* EVOLUCIONES (sin cambios estructurales) */}
            <div className="flex-1 border border-zinc-800 rounded-xl bg-zinc-900/50 overflow-hidden flex flex-col shadow-lg">
                ...
                <div className="p-4 border-b border-zinc-800 bg-zinc-950/80 space-y-4 animate-in slide-in-from-top-2 duration-200">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <textarea value={noteFields.subjetivo} onChange={(e) => setNoteFields((p) => ({ ...p, subjetivo: e.target.value }))} rows={4} className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-sm text-zinc-200 focus:border-blue-500 outline-none resize-none" placeholder="Motivo de consulta y síntomas..." />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-emerald-400 uppercase">Objetivo (O)</label>
                      <textarea value={noteFields.objetivo} onChange={(e) => setNoteFields((p) => ({ ...p, objetivo: e.target.value }))} rows={4} className="w-full rounded-lg bg-zinc-900 border border-zinc-700 px-3 py-2 text-sm text-zinc-200 focus:border-emerald-500 outline-none resize-none" placeholder="Signos vitales y examen físico..." />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                  ...
                  <div className="flex justify-end pt-2">
                    <button onClick={handleCreateNote} disabled={noteLoading || !activeUser} className="rounded-lg bg-emerald-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-900/20 transition-all flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                {noteLoading && !showNewNote ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-zinc-600"><span className="text-4xl mb-2 opacity-20">📝</span><p>No hay evoluciones registradas</p></div>
                ) : (
                  notes.map((note) => (
                    <div key={note.id} onClick={() => selectNote(note.id === selectedNote?.id ? null : note)} className={`p-4 rounded-xl border cursor-pointer transition-all duration-200 ${selectedNote?.id === note.id ? "bg-blue-900/10 border-blue-500/30 shadow-md" : "bg-zinc-950/50 border-zinc-800 hover:bg-zinc-800/50 hover:border-zinc-700"}`}>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-xs text-zinc-500 font-mono bg-zinc-900 px-2 py-1 rounded">{new Date(note.created_at).toLocaleString([], { dateStyle: "short", timeStyle: "short", hour12: false })}</span>
                      </div>
                      ...
                      {selectedNote?.id === note.id && note.fields && (
                        <div className="mt-4 pt-4 border-t border-zinc-800 grid grid-cols-2 gap-x-6 gap-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
                          {[{ k: "subjetivo", l: "S", c: "blue" }, { k: "objetivo", l: "O", c: "emerald" }, { k: "evaluacion", l: "A", c: "amber" }, { k: "plan", l: "P", c: "purple" }].map((s) => (
                            <div key={s.k} className="col-span-2 sm:col-span-1 space-y-1">
