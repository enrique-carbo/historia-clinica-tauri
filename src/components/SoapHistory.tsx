  ...
  return (
    <div className="flex flex-col gap-5 mt-2.5">
      {records.map((record) => (
        ...
        >
          {/* Cabecera con Fecha, Firma y Badge de Sync */}
          <div className="flex justify-between items-center border-b border-zinc-800 pb-2.5 mb-3">
            ...
            <div className="flex items-center gap-2">
              {/* NUEVO: Escudo de Verificación Criptográfica */}
              {record.is_verified ? (
                <span className="text-[11px] px-2 py-0.5 rounded-full font-semibold bg-emerald-950 text-emerald-400 border border-emerald-900">
                  ✔ Firma Válida
                </span>
              ) : (
                <span className="text-[11px] px-2 py-0.5 rounded-full font-semibold bg-red-950 text-red-400 border border-red-900">
                  ⚠ Alterado
                </span>
              )}

              {/* Badge de Sincronización (El que ya tenías) */}
              <span
